...
impl<S> SaveBuilder<S> {
    ...
    #[doc(hidden)]
    pub fn new(saveable: S) -> SaveBuilder<S> {
    ...
    M: ReadEntry,
{
    ...
    pub fn with_entries(self, mut entries: Entries) -> EntriesSaveResult<M> {
        ...
        let mut res = ReadEntry::read_entry(saveable);
    ...
    MultipartData<M>: BufRead,
{
    ...
    pub fn write_to<W: Write>(&mut self, mut dest: W) -> SaveResult<u64, u64> {
        if self.size_limit < u64::MAX {
            ...
            try_read_buf(&mut self.saveable, |buf| try_write_all(buf, &mut dest))
        }
    }
    ...
    fn read_mem<Wb: FnMut(&[u8]) -> SaveResult<usize, usize>>(
        ...
        try_copy_limited(&mut self.saveable, with_buf, limit)
    }
